...
def create_vector_index():
    """Create a Redis search index for JSON documents with vectors using HNSW."""
    try:
        # Define schema
        schema = (
            ...
            VectorField(
                "$.vector",
                "HNSW",  # Changed to HNSW
                {
                    "TYPE": "FLOAT32",
                    "DIM": 1024,  # For bge-large-en-v1.5
                    "DISTANCE_METRIC": "COSINE",
                    "M": 16,  # Number of neighbors per layer
                    "EF_CONSTRUCTION": 200,  # Size of dynamic list during construction
                    "EF_RUNTIME": 10  # Size of dynamic list during search
                },
                as_name="vector"
            )
        )
